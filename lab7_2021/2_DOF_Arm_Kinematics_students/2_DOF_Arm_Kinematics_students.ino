...
float CircleCenterX = 0.20;
float CircleCenterY = 0.00;
float Radius = 0.07;
...
float pwm;
float vc;
float set_point_1; // Set point (desired position) for the motor 1
float set_point_2; // Set point (desired position) for the motor 2
...
// Circular Path
    ...
    x_e = 0.16*sq(sin(i*(PI/18)))*sin(i*(PI/18))+CircleCenterX;
    y_e = 0.12*cos(i*(PI/18))-0.05*cos(2*i*(PI/18))-0.02*cos(3*i*(PI/18))-0.01*cos(4*i*(PI/18))+CircleCenterY;
...
#ifdef CIRCLE
    x_e = Radius*cos(i*(PI/18))+CircleCenterX;
    y_e = Radius*sin(i*(PI/18))+CircleCenterY;
...
void Inverse_K()
{
  ...
  q_2_ik = 2*atan2(sqrt(sq(l_1+l_2)-(sq(x_e)+sq(y_e))),sqrt((sq(x_e)+sq(y_e))-sq(l_1-l_2)));
